...
- **Filesystem-view caveat when YOU (the orchestrator) run sandboxed.** `marker_verified`/`artifacts_present` means `codex.py` `stat`'d the `--expect` files in its OWN process at completion. If your orchestrator runs in a different filesystem sandbox than `codex.py`/Codex, the two views can diverge **both ways** — observed live when driving `codex.py` from a sandboxed sub-Claude: (a) a file that is really on disk looked *missing* to the orchestrator's foreground Bash; and (b) a file `codex.py` reported verified did *not* durably persist into the orchestrator's real FS after the pane auto-closed. So: don't treat a single foreground re-`stat` as authoritative in either direction; when the artifact must persist for inspection, pass **`--no-close`** so the Codex pane (and its writes) stay put, and verify in the same environment that will consume the file. In a normal (non-sandboxed) setup `codex.py`'s on-disk verification matches your view.
